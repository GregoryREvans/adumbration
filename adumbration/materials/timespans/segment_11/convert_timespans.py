...
from adumbration.materials.score_structure.segment_11.rhythm_material_pattern import (
    rhythm_material_list,
)
from adumbration.materials.score_structure.segment_11.time_signatures import bounds
...
segment_11_rhythm_timespans = evans.timespan.make_split_list(
    rhythm_timespan_list, bounds
)

rhythm_commands = []
for span in segment_11_rhythm_timespans:
...
segment_11_pitch_timespans = pitch_timespan_list

pitch_commands = []
for span in segment_11_pitch_timespans:
...
segment_11_dynamic_timespans = dynamic_timespan_list

dynamic_commands = []
for span in segment_11_dynamic_timespans:
    command = evans.HandlerCommand(
        voice_name=span.voice_name,
        timespan=abjad.Timespan(span.start_offset, span.stop_offset),
        handler=span.handler,
...
segment_11_notehead_timespans = notehead_timespan_list

notehead_commands = []
for span in segment_11_notehead_timespans:
    command = evans.HandlerCommand(
        voice_name=span.voice_name,
        timespan=abjad.Timespan(span.start_offset, span.stop_offset),
        handler=span.handler,
